...
public class CalculateWheelRadiusCommand extends Command {
    ...
    public CalculateWheelRadiusCommand(SwerveDrive drive, double assumedWheelRadius, double driveBaseRadius) {
        this.drive = drive;
    ...
    public void execute() {
        if (!timer.hasElapsed(1)) {
            drive.drive(
                    new ChassisSpeeds(0, 0, thetaRateLimiter.calculate(1)
                    ));
        }
        else {
            drive.drive(
                    new ChassisSpeeds(0, 0, thetaRateLimiter.calculate(0)
                    ));
        }
    }
    ...
    public boolean isFinished() {
        return timer.hasElapsed(1) && drive.getChassisSpeeds().omegaRadiansPerSecond < 0.001;
    }
    ...
    public void end(boolean interrupted) {
        ...
        double actualDistanceMeters = Math.abs(startYaw.getRadians() - endYaw.getRadians()) * driveBaseRadius;
