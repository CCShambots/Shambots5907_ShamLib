...
public class AutoStatusCommand extends Command {
  ...
  /**
   * Command that splits the lights up into equal segments based on a number of conditions. Should
   * generally be run before the autonomous period to assure drivers that the robot is ready to run.
   *
   ...
   * @param numLEDs The total number of LEDs to run the animation across (not including those in the
   *     startOffset)
   * @param startOffset Starting offset, should default to 8 in most use cases (the number of LEDs
   *     on the CANdle)
